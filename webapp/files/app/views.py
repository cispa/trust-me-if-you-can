...
def response(request, template, context=None):
    ...
    # Comment in the line below to enforce Trusted Types (see static/trusted-types.js)
    # r.headers['content-security-policy'] = "trusted-types default; require-trusted-types-for 'script';"
    return r
...
def index(request):
    ...
    return response(request, 'content.html', context={'notes': notes})
...
def mensa(request):
    # Render about page
    return response(request, 'mensa.html')
...
def about(request):
    # Render about page
    return response(request, 'about.html')
